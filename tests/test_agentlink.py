...
SKILL_DIR = ROOT / "skills" / "agentlink"
SCRIPT = SKILL_DIR / "scripts" / "agentlink.py"


class AgentlinkTest(unittest.TestCase):
    def test_packaged_skill_layout(self) -> None:
        skill = SKILL_DIR / "SKILL.md"

        self.assertTrue(skill.is_file())
        self.assertTrue(SCRIPT.is_file())
        self.assertIn("name: agentlink", skill.read_text(encoding="utf-8"))
    ...
    def test_global_skills_skip_codex_symlink_entries(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            home = Path(tmp)
            self.make_codex_skill_symlink(home, home / ".codex" / "skills")
    ...
    def test_global_skills_skip_stale_codex_symlink_entries(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            home = Path(tmp)
            self.make_codex_skill_symlink(home, home / ".codex" / "skills", create_source=False)
    ...
    def test_repo_skills_skip_codex_symlink_entries(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            self.make_repo(repo)
            self.make_codex_skill_symlink(repo, repo / ".agents" / "skills")
    ...
    def test_repo_skills_skip_stale_codex_symlink_entries(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            self.make_repo(repo)
            self.make_codex_skill_symlink(repo, repo / ".agents" / "skills", create_source=False)
    ...
    def test_plugin_skills_command_is_invalid(self) -> None:
        result = self.run_sync("global", "plugin-skills")
